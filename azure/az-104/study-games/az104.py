#!/usr/bin/env python3
"""Microsoft Azure Administrator AZ-104 terminal study center.

Run: python3 az104.py
"""
import importlib
import os
import pkgutil
...
DOMAINS = {
    ...
    5: ("Monitoring & Maintenance", "10-15"),
}

# 50 questions split by official domain weight (20-25 / 15-20 / 20-25 / 15-20 / 10-15)
EXAM_SPLIT = {1: 12, 2: 10, 3: 12, 4: 9, 5: 7}
EXAM_MINUTES = 100
...
def main_menu(mods, progress):
    banner("MICROSOFT AZURE ADMINISTRATOR AZ-104 STUDY CENTER")
    total_q = sum(len(m.QUESTIONS) for m in mods)
    print(wrap(f"{len(mods)} topics | {total_q} questions | infinite subnetting, "
               f"RBAC, NSG, and HA drills | 50-question exam simulator"))
    by_domain = {}
    ...
    print("    E) Exam simulator (50 questions, weighted, timed)")
    print("    G) Azure gym (subnetting, RBAC, NSG rules, HA math - infinite)")
...
def exam_sim(mods, progress):
    ...
    print(wrap(f"50 questions weighted like the real exam ({', '.join(f'D{d} {n}' for d, n in EXAM_SPLIT.items())}). "
    ...
    for i, (d, tid, q) in enumerate(paper, 1):
        ...
        clock = (C.RED if left < 15 else C.DIM) + f"[{elapsed:.0f} min elapsed]" + C.RESET
        print(f"\n{clock}")
