#!/usr/bin/env python3
"""CompTIA Network+ N10-009 terminal study center.

Run: python3 netplus.py
"""
import importlib
import os
import pkgutil
...
# 90 questions split by official domain weight
EXAM_SPLIT = {1: 21, 2: 18, 3: 17, 4: 13, 5: 21}
EXAM_MINUTES = 90
...
def main_menu(mods, progress):
    ...
    print("    S) Subnetting gym (infinite generated problems)")
    print("    P) Port blitz")
...
def exam_sim(mods, progress):
    ...
    print(wrap(f"90 questions weighted like the real exam ({', '.join(f'D{d} {n}' for d, n in EXAM_SPLIT.items())}). "
               ...
               "informational. CompTIA passing score is 720 on a 100-900 scale — "
    ...
    for d, (name, pct) in DOMAINS.items():
        ...
        bar = "█" * int(dp / 5)
        print(color + f"  {d}.0 {name:<26} {score[d]:>2}/{asked[d]:<2} {dp:>3.0f}% {bar}" + C.RESET)
    print()
    print(engine.grade_line(total_score, total_asked))
    pct = 100.0 * total_score / total_asked
    verdict = ("PASS territory — you would likely clear 720/900."
               if pct >= 80 else
...
def main():
    ...
    while True:
        ...
        try:
            ...
            choice = "Q"
        if choice == "Q":
            print(C.CYAN + "\n  Good luck on the exam! (720 to pass — you've got this)\n" + C.RESET)
            return
        try:
            if choice == "E":
